...
def add_noise(signal: np.ndarray, 
    ...
    """
    Add noise augmentation to an audio signal.
    ...
    Returns:
        np.ndarray: The signal with noise added (or original signal if noise params are invalid)
    """
    if noise_folder is None or snr_db is None or not os.path.exists(noise_folder):
        return signal
...
def main(subdir: str, 
    ...
    signal = add_noise(signal, sample_rate, noise_folder, snr_db)
